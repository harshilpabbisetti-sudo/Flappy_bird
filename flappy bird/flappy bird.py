...
class Owl(pygame.sprite.Sprite):
    ...
    def update(self):
        global game_phase, transparency, invincibility, lives, start_time, high_score, prev_phase
        if self.rect.y < 0 or self.rect.y > 350:                        # checks for death
            ...
            if lives > 1:
                ...
                if high_score < score:
                    cursor.execute(
                        f"update login_credentials set {modes[prev_phase]}_high_score = {int(score)} where username = '{user}'")
                    mycon.commit()
                    high_score = score
            else:
...
class Wave(pygame.sprite.Sprite):
    def __init__(self):
        ...
        for i in range(1, 5):                               # creates a nested list for every set of wave animation
            self.waves_list.append([pygame.image.load(f'graphics/waves/Waves-set-{i}/{i}{j}.png').convert_alpha() for j in range(1, 7)])
...
def collision():
    global invincibility, lives, start_time, transparency, game_phase, high_score
    if pygame.sprite.spritecollide(player.sprite, obstacle_group, False) and not invincibility:
        if lives > 1:
            ...
            if high_score < score:
                cursor.execute(
                    f"update login_credentials set {modes[game_phase]}_high_score = {int(score)} where username = '{user}'")
                mycon.commit()
                high_score = score
...
if mycon.is_connected():
    ...
    lost_surf = font.render('Oops, You Lost!!', False, (111, 196, 169))
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == obstacle_timer and game_phase in [2, 3, 4]:          # creation of pillars
                ...
                obstacle_group.add(Obstacle(pillar_xpos, height + 350))           # 350 - height of pillar is the set space bw pillars
        ...
        if game_phase in [2, 3, 4]:
            ...
            if game_phase == 0:
                ...
                if game_start:
                    end_score()
                    screen.blit(lost_surf, game_name_rect)
                else:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if easy_rect.collidepoint(event.pos):
                        game_phase = 2
                        cursor.execute(f"select {modes[game_phase]}_high_score from login_credentials where username = '{user}'")
                    elif moderate_rect.collidepoint(event.pos):
                        game_phase = 3
                        cursor.execute(f"select {modes[game_phase]}_high_score from login_credentials where username = '{user}'")
